...
filtered_path = 'outputs/lms_filtered_fix/';
Fs_target = 16000;
doListen  = false;     
doFigure  = true;      
saveCSV   = true;
...
if isempty(mixed_files), error('Không thấy file mixed'); end
if isempty(clean_files), error('Không thấy file clean'); end
...
fprintf('\n KẾT QUẢ ĐÁNH GIÁ LMS FILTER (toàn bộ mixed)\n');
fprintf('----------------------------------------------------------------------------------------------\n');
...
for i = 1:length(mixed_files)
    ...
    f_clean = '';
    if isKey(cleanMap, clean_key), f_clean = cleanMap(clean_key); end
    f_noisy = fullfile(mixed_path, [base '.wav']);
    f_filt  = fullfile(filtered_path, [base '_lms.wav']);

    if isempty(f_clean) || ~isfile(f_noisy) || ~isfile(f_filt), continue; end
    ...
    L = min([length(xc), length(xn), length(xf)]);
    xc = xc(1:L); xn = xn(1:L); xf = xf(1:L);

    e_in  = xn - xc;
    e_out = xf - xc;
    snr_b = 10*log10(sum(xc.^2)/(sum(e_in.^2)+eps));
    snr_a = 10*log10(sum(xc.^2)/(sum(e_out.^2)+eps));
    dSNR  = snr_a - snr_b;
...
if saveCSV && ~isempty(M)
    ...
    outCSV = fullfile('outputs','lms_compare_metrics.csv');
    writetable(T, outCSV);
    fprintf('Đã lưu CSV: %s\n', outCSV);
end

% Nghe & vẽ 1 mẫu (ΔSNR lớn nhất)
if (doFigure || doListen) && exist('T','var') && ~isempty(T)
    ...
    if isfile(f_clean) && isfile(f_noisy) && isfile(f_filt)
        ...
        if doListen
            pause_dur = zeros(round(0.3*Fs_target),1);
            try
                sound([xn; pause_dur; xf; pause_dur; xc], Fs_target);
            catch
                warning('Không phát được âm thanh.');
            end
        end

        if doFigure
            figure('Name',['LMS So sánh: ' strrep(base,'_','\_')], 'NumberTitle','off','Position',[100 100 1000 750]);
            ...
            subplot(3,2,2); spectrogram(xn,256,128,256,Fs_target,'yaxis'); title('Noisy Spec');
            subplot(3,2,4); spectrogram(xf,256,128,256,Fs_target,'yaxis'); title('Filtered Spec');
            subplot(3,2,6); spectrogram(xc,256,128,256,Fs_target,'yaxis'); title('Clean Spec');
            sgtitle(['So sánh LMS: ' strrep(base,'_','\_')]);
        end
    end
end
